...
lazy_static! {
  ...
  // Re-encoded to Ristretto externally as dalek does not provide a method to do so
  // (modified dalek enabling constructing a RistrettoPoint out of an EdwardsPoint)
  static ref ALT_BASEPOINT: RistrettoPoint = {
    CompressedRistretto(hex!("683e50157320142f1f2449b791a419346a234eef9fb878bf0cfe2b17ee012146")).decompress().unwrap()
  };
}
...
impl CryptEngine for RistrettoEngine {
  ...
  fn bytes_to_signature(bytes: &[u8]) -> anyhow::Result<Self::Signature> {
    ...
    #[allow(non_snake_case)]
    let R = CompressedRistretto::from_slice(&bytes[..32]).decompress().ok_or(anyhow::anyhow!("Invalid point in signature specified"))?;
    Ok(Signature {
  ...
  fn dl_eq_generate_commitments(key: [u8; 32]) -> anyhow::Result<Vec<Commitment<Self>>> {
    ...
    let pubkey = &Scalar::from_bytes_mod_order(key) * &RISTRETTO_BASEPOINT_TABLE;
    debug_assert_eq!(
  ...
  fn sign(key: &Self::PrivateKey, message: &[u8]) -> anyhow::Result<Self::Signature> {
    ...
    let A = key * &RISTRETTO_BASEPOINT_TABLE;
    let mut hram = [0u8; 64];
    let hash = blake2::Blake2b::new()
  ...
  fn verify_signature(public_key: &Self::PublicKey, message: &[u8], signature: &Self::Signature) -> anyhow::Result<()> {
    ...
    hram.copy_from_slice(&hash);
    let c = Scalar::from_bytes_mod_order_wide(&hram);
    let expected_R = &signature.s * &RISTRETTO_BASEPOINT_TABLE - c * public_key;
  ...
  fn encrypted_sign(
    ...
    let pub_nonce = &nonce * &RISTRETTO_BASEPOINT_TABLE;
    let challenge_nonce = &pub_nonce + encryption_key;
    let pub_signing_key = signing_key * &RISTRETTO_BASEPOINT_TABLE;
    let mut hram = [0u8; 64];
    let hash = blake2::Blake2b::new()
  ...
  fn encrypted_verify(
    ...
    if expected_sB != &ciphertext.s * &RISTRETTO_BASEPOINT_TABLE {
  ...
  fn decrypt_signature(sig: &Self::EncryptedSignature, key: &Self::PrivateKey) -> anyhow::Result<Self::Signature> {
    ...
    let sig = Signature {
      s: sig.s + key,
      R: sig.R + key * &RISTRETTO_BASEPOINT_TABLE,
    };
    trace!(
      "Decrypting signature with encryption pubkey {}: {} -> {}",
      hex::encode((key * &RISTRETTO_BASEPOINT_TABLE).compress().as_bytes()),
      hex::encode(Self::encrypted_signature_to_bytes(&sig)),
  ...
  fn recover_key(expected_key: &Self::PublicKey, ciphertext: &Self::EncryptedSignature, sig: &Self::Signature) -> anyhow::Result<Self::PrivateKey> {
    ...
    if &(&key * &RISTRETTO_BASEPOINT_TABLE) != expected_key {
